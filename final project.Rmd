...
\begin{equation}
  \hat\beta_{3} =
    \begin{cases}
      \beta_{average_hours_worked0.1 to 29.9 hours} = 1.831046 & \text{if average hours worked is 0.1-29.9 hours}\\
      \beta_{average_hours_worked30.0 to 40.0 hours} = 1.973047 & \text{if average hours worked is 30-40 hours}\\
      \beta_{average_hours_worked40.1 to 50.0 hours} = 2.117700  & \text{if average hours worked is 40.1-50 hours}\\
      \beta_{average_hours_worked50.1 hours and more} = 2.157288 & \text{if average hours worked is 50.1 and more hours}\\
      \beta_{average_hours_workedDon't know} = 1.343838 & \text{if average hours worked is not known}\\
...
\begin{equation}
  \hat\beta_{4} =
    \begin{cases}
      \beta_{marital_statusLiving common-law} =  -0.545830 & \text{if marital status is having living common-law partner}\\
      \beta_{marital_statusMarried} = -0.462328 & \text{if marital status is married}\\
      \beta_{marital_statusSeparated} = -1.177676  & \text{if marital status is separated}\\
      \beta_{marital_statusSingle, never married} = -1.096904 & \text{if marital status is single and never married}\\
      \beta_{marital_statusWidowed} = -0.609398  & \text{if marital status is widowed}\\
...
\begin{equation}
  \hat\beta_{5} =
    \begin{cases}
      \beta_{educationCollege, CEGEP or other non-university certificate or diploma} =  0.143529 & \text{if marital status is having living common-law partner}\\
      \beta_{educationHigh school diploma or a high school equivalency certificate} = 0.193145 & \text{if education level is as stated}\\
      \beta_{educationLess than high school diploma or its equivalent } = 0.527132  & \text{if education level is as stated}\\
      \beta_{educationTrade certificate or diploma} = 0.151863  & \text{if education level is as stated}\\
      \beta_{educationUniversity certificate or diploma below the bachelor's level} = 0.294567 & \text{if education level is as stated}\\
      \beta_{educationUniversity certificate, diploma or degree above the bachelor} = 0.020526    & \text{if education level is as stated}\\
...
\begin{equation}
  \hat\beta_{6} =
    \begin{cases}
      \beta_{income_family \$125,000 and more} =  0.199942\\
      \beta_{income_family \$25,000 to \$49,999} = -0.166061\\
      \beta_{income_family \$50,000 to $74,999} = 0.042703\\
      \beta_{income_family \$75,000 to \$99,999} = -0.022085\\
      \beta_{income_familyLess than \$25,000} = -0.106366\\
...
\begin{equation}
  \hat\beta_{7} =
    \begin{cases}
      \beta_{self_rated_healthExcellent} =  1.050221\\
      \beta_{self_rated_healthFair} = 0.186045 \\
      \beta_{self_rated_healthGood} = 0.810536\\
      \beta_{self_rated_healthPoor} = 0.047775\\
      \beta_{self_rated_healthVery good} =  0.829585 \\
...
\begin{equation}
  \hat\beta_{8} =
    \begin{cases}
      \beta_{self_rated_mental_healthExcellent } =  4.478973\\
      \beta_{self_rated_mental_healthFair} = 2.406130  \\
      \beta_{self_rated_mental_healthGood} = 3.387906 \\
      \beta_{self_rated_mental_healthPoor} = 0.389899\\
      \beta_{self_rated_mental_healthVery good} =  4.036514 \\
      0 & \text{if self rated mental health is other}
    \end{cases}       
\end{equation}
And $\hat\beta_{9} =  -0.043339$, which is the estimated slope for age_youngest_child_under_6, with p-value = 0.019520 < 0.05, which is significnat. $\hat\beta_{10} =  0.054027$, which is the estimated slope for total_children, with p-value = 0.102176 > 0.05. All slope estimators and corresponding p-values are listed in the table below.\
